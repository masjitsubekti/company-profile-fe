<template>
  <div>
    ...
    <b-modal size="lg" :title="titleModal" scrollable centered v-model="showModal" :no-close-on-backdrop="true">
      <b-container>
        <JenisProjectForm :dataForm="dataForm" ref="refJenisProjectForm" @reloadData="reloadData" />
      </b-container>
...
export default {
  ...
  methods: {
    getAll() {
      this.isBusy = true
      jenisProjectUseCase.getAll().then((response) => {
        if (!response.error) {
    ...
    prosesDelete(val) {
      jenisProjectUseCase.deleteData(val.id).then((response) => {
        if (!response.error) {
